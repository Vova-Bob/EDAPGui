...
class NavRouteParser:
    ...
    FALLBACK_MESSAGES = {
        'log.navroute.read_error': "An error occurred reading {resource} file. File may be open. {error}",
        'log.status.retry_attempt': "Attempting to re-read {resource} after {delay}s delay.",
    }
    ...
    def get_nav_route_data(self):
        ...
        while True:
            try:
                ...
                self._log('log.navroute.read_error', error=str(e), resource=self.file_path)
                sleep(backoff)
                self._log('log.status.retry_attempt', resource=self.resource_name, delay=backoff)
                backoff *= 2
    ...
    def _log(self, key: str, **kwargs):
        if self._log_func:
            try:
                self._log_func(key, **kwargs)
                return
            except Exception as exc:
                logger.debug(f"Failed to emit localized log '{key}': {exc}")

        message = self.FALLBACK_MESSAGES.get(key)
        if message:
            try:
