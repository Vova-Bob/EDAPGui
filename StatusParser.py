...
class StatusParser:
    ...
    def log_flag_diffs(self):
        ...
        for item in flag_array:
            self._log('log.status.flag.on', level='debug', flag=item)

        flags_off = ~ new_flags & old_flags
        flag_array = self.translate_flags(flags_off)
        for item in flag_array:
            self._log('log.status.flag.off', level='debug', flag=item)
        ...
        for item in flag_array2:
            self._log('log.status.flag2.on', level='debug', flag=item)

        flags_off2 = ~ new_flags2 & old_flags2
        flag_array2 = self.translate_flags2(flags_off2)
        for item in flag_array2:
            self._log('log.status.flag2.off', level='debug', flag=item)
