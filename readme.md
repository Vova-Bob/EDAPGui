...
* Базовий рушій pyttsx3 залишається основним і використовується завжди, якщо нейронний синтез відсутній або не працює.
* Щоб увімкнути нейронний голос:
  * скористайтеся секцією «Голос» у налаштуваннях GUI (виберіть мову озвучення "uk", увімкніть «український нейронний TTS», оберіть голос),
  * або додайте в `configs/AP.json`: `"UkrainianNeuralTTS": true`, `"VoiceLanguage": "uk"`, `"UAVoice": "Dmytro"` (чи інший, підтримуваний бібліотекою).
  * бібліотека `ukrainian-tts` не входить до основного списку залежностей; за потреби встановіть її вручну разом із залежностями SciPy.
* Використовується бібліотека [ukrainian-tts](https://github.com/robinhad/ukrainian-tts) — подяка авторові за проєкт.
