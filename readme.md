...
```
Налаштування `VoiceLanguage` і вибір голосу тепер можна змінювати через розділ «Голос» у GUI (окремо від мови OCR/UI).

### Український TTS через RHVoice
* Основний рушій — лише системні голоси через pyttsx3; жодні додаткові ML-моделі не потрібні.
* Інсталяторів у цьому репозиторії немає: Windows-голоси RHVoice завантажуйте з офіційної сторінки українських голосів `https://rhvoice.org/uk-voices/`, де розміщені посилання на актуальні збірки.
  * На цій сторінці переходьте до розділу з **Windows**-інсталяторами та обирайте `.exe`, що явно позначений як setup/installer (наприклад, `RHVoice-setup.exe`). Посилання на вихідний код (`source code`) не встановлюють голоси в систему.
  * Стислий гайд по встановленню у Windows:
    1. Завантажте інсталятор із `https://rhvoice.org/uk-voices/` і запустіть його, залишивши типову опцію **Install for all users** (додає SAPI-голоси в систему).
    2. У майстрі вибору мов переконайтеся, що позначено **Ukrainian** (або додайте потрібні голоси через «Add voices» у Windows після встановлення).
    3. Після завершення встановлення перезавантажте Windows або хоча б перелогіньтесь, щоб SAPI-голоси з'явилися у списку системних.
    4. Для перевірки відкрийте **Settings → Time & Language → Speech** і переконайтеся, що українські голоси від RHVoice відображаються серед **Voices** (Anatol, Natalia, Volodymyr, Marianna тощо).
* Перезапустіть автопілот, відкрийте розділ «Голос» і:
  * увімкніть опцію озвучення,
  * виберіть мову озвучення `uk`,
  * оберіть потрібний голос зі списку доступних системних голосів.
* RHVoice встановлюється окремо від Python-залежностей; стандартний `requirements.txt` містить лише базові пакети (pyttsx3 тощо).
* Якщо використовуєте власні експериментальні скрипти або нейронні моделі, тримайте їх окремо — стандартна збірка їх не вимагає.
